...
// --- FIXED: ADDED REDIS MOCK TO PREVENT CACHE HITS ---
...
describe("Tenant Controller", () => {
  ...
  describe("getTenantDetail", () => {
      test("should fetch tenant details and return 200", async () => {
        // 1. USE UNIQUE IDs TO BYPASS ANY REAL REDIS CACHE
        mockReq.user.id = "user-brand-new-123";
        mockReq.oidc.user.sub = "auth0|brand-new-123";
  
        const fakeUser = {
          id: "user-brand-new-123",
          role: "Admin",
          tenant: { id: "tenant-brand-new-99" },
        };
        mockSecuredDb.user.findUnique.mockResolvedValue(fakeUser);
  
        const fakeTenantDetail = {
          id: "tenant-brand-new-99",
          name: "Zappotel NYC",
          slug: "zappotel-nyc",
          currency: "USD",
          user: { firstName: "Alex" },
        ...
        expect(mockSecuredDb.user.findUnique).toHaveBeenCalledWith({
          where: { id: "user-brand-new-123" },
          include: { tenant: true },
        });
  
        // 2. KEEPS YOUR OBJECT CONTAINING FIX
        expect(mockSecuredDb.tenant.findUnique).toHaveBeenCalledWith(
          expect.objectContaining({
            where: { userId: "user-brand-new-123" },
          }),
        );
      ...
      test("should throw 404 if user is not found in database", async () => {
        // UNIQUE ID TO BYPASS CACHE
        mockReq.oidc.user.sub = "auth0|not-found-123";
      ...
      test("should throw 404 if user is found, but tenant details are missing", async () => {
        // UNIQUE IDs TO BYPASS CACHE
        mockReq.user.id = "user-missing-tenant-123";
        mockReq.oidc.user.sub = "auth0|missing-tenant-123";
  
        mockSecuredDb.user.findUnique.mockResolvedValue({
          id: "user-missing-tenant-123",
          role: "Admin",
          tenant: { id: "tenant-missing-99" },
        });
